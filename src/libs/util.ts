...
//  通过路由列表得到菜单列表
export const getMenuByRouter = (list: RouteConfig[], access: string[]) => {
  let res: Array<{
    name?: string;
    icon: string;
    href?: string;
...
export const getBreadCrumbList = (route: Route, homeRoute) => {
 ...
 */
export const setTagNavListInLocalstorage = (list: Route[]) => {
  localStorage.tagNaveList = JSON.stringify(list);
};
/**
 * @returns {Array} 其中的每个元素只包含路由原信息中的name, path, meta三项
 */
export const getTagNavListFromLocalstorage = (): Route[] => {
 ...
 */
export const getNewTagList = (list: any[], newRoute: RouteConfig) => {
 ...
 */
export const getNextRoute = (list: Route[], route: Route) => {
  let res = {};
  if (list.length === 2) {
...
export const getArrayFromFile = file => {
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = function(evt) {
      let data = (evt.target as FileReader).result; // 读到的数据
      let pasteData = (data! as string).trim();
      arr = pasteData
        .split(/[\n\u0085\u2028\u2029]|\r\n?/g)
        .map(row => {
          return row.split("\t");
...
export const findNodeUpperByClasses = (ele: Element, classes: string[]) => {
  // 上层元素元素是包含classes
 ...
 */
export const setTitle = (routeItem: Route, vm) => {
